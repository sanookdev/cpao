...
class Rememberme {
	...
	function setCookie($netid = "", $nocookie = false) {
		...
		$nocookie ? $this->CI->db->where('php_session_id', session_id()) : 
					$this->CI->db->where('cookie_id', $this->CI->input->cookie('rmtoken_' . str_replace('.', '_', $_SERVER['SERVER_NAME']), TRUE));
		...
		if (!$nocookie) {
			// set cookie for 1 year = 31557600
			// 30 minutes = 30 * 60
			$cookie = array(
	...
	function deleteCookie() {
		...
		$query = $this->CI->db->get_where('ci_cookies', array(
			'cookie_id' => $this->CI->input->cookie('rmtoken_' . str_replace('.', '_', $_SERVER['SERVER_NAME']), TRUE)
		));
