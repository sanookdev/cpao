...
$is_admin = isset($_GET['p']) && $_GET['p'] == 'qwe23dvsdt$@FSDry57782';
...
$opts = array(
	...
	'roots' => array(
		...
		array(
			...
			'path'          => '../../../awe12rhsto1m23fsd/' . $path,                 // path to files (REQUIRED)
			'URL'           => '/cpao/download/finder?file=' . $path, // URL to files (REQUIRED)
			'trashHash'     => 't1_Lw',                     // elFinder's hash of trash folder
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
			'uploadDeny'    => array('all'),                // All Mimetypes not allowed to upload
			'uploadAllow'   => array('application/zip', 'application/x-zip-compressed', 'application/pdf', 'application/msword', 'image/x-ms-bmp', 'image/gif', 'image/jpeg', 'image/png', 'image/x-icon', 'text/plain'), // Mimetype `image` and `text/plain` allowed to upload
...
if ($is_admin && $path == 'Drive') {
	// Trash volume
	$opts['roots'][] = array(
		'id'            => '1',
		'driver'        => 'Trash',
		'path'          => '../../../awe12rhsto1m23fsd/Drive/.trash/',
		'tmbURL'        => '../../awe12rhsto1m23fsd/Drive/.trash/.tmb/',
		'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
		'uploadDeny'    => array('all'),                // Recomend the same settings as the original volume that uses the trash
		'uploadAllow'   => array('application/zip', 'application/x-zip-compressed', 'application/pdf', 'application/msword', 'image/x-ms-bmp', 'image/gif', 'image/jpeg', 'image/png', 'image/x-icon', 'text/plain'), // Same as above
