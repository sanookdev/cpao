		...
		<div id="content">
			...
			<div class="row">
				<div class="col-xs-12 mb-1">
					<button id="btnadd" name="btnadd" class="btn btn-primary">เพิ่มเมนูย่อย</button>
				</div>
			</div>
			<!-- widget grid -->
										...
										<table id="table<?= $row['menu_id'] ?>" name="menu" data-id="<?= $row['menu_id'] ?>" class="table table-striped table-bordered table-hover" width="100%">
											<thead>
												<tr>
													...
													<th data-class="expand"><i class="fa fa-fw fa-cube text-muted hidden-md hidden-sm hidden-xs"></i> เมนูย่อย</th>
												</tr>
											</thead>
											<tbody>
											</tbody>
						...
						<fieldset>
							...
							<div class="row">
								<section class="col col-8">
									<label class="label">URL หรือ นามแฝง</label>
							...
							<div class="row">
								<section class="col col-6 field-page">
									<label class="label"><a id="page" href="javascript:void(0)">ไปที่หน้า</a> | <a id="load_page" href="javascript:void(0)">โหลดใหม่</a></label>
									<select id="page_id" style="width:100%" class="select2" name="page_id">
										<option value="" selected="">เลือก หน้า</option>
										<?php
										foreach ($list_page as &$row) { ?>
											<option value="<?= $row['page_id'] ?>"><?= $row['page_title'] ?> (<?=$row['page_type']?>)</option>
										<?php	} ?>
									</select> <i></i>
								</section>
	...
	<script>
		$(document).ready(function() {
			...
			$(document).on('click', 'button[name=btnedit]', function() {
				var id = $(this).data('id')
				typeModal = 'edit'
				$('.modal-title').html('แก้ไขเมนูย่อย')
				$.get('<?= base_url() ?>AdminManageMenu/menu?id=' + id, function(data) {
				    ...
				    $('#page').html(data.page_id ? 'แก้ไขหน้า' : 'เพิ่มหน้า')
			...
			$('#load_page').click(function() {
				$.get('<?= base_url() ?>AdminManageMenu/load_page', function(data) {
					...
					for (var i in data) {
						var item = data[i]
						html += `<option value="${item.page_id}">${item.page_title} (${item.page_type})</option>`
					}
					$('#page_id').html(html)
					$('#page_id').trigger('change')
				})
