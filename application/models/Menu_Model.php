...
class Menu_Model extends CI_Model
{
    ...
    public function menu($id)
    {
        $this->db->select('menu.*, p.parent_id as parent_parent_id, p.menu_title as parent_menu_title, 
            (select count(*) from menu s where s.parent_id = menu.menu_id and s.is_delete = 0) as count_child');
    ...
    public function getPageBy($by, $value, $parent_not_null = false, $page_is_null = false, $is_cache = false)
    {
        if ($is_cache) {
            $this->db->cache_on();
        }
        ...
        $this->db->where('menu.' . $by, $value);
        $this->db->where('menu.is_delete', 0);
        $this->db->where("page.page_type != 'link'", NULL, FALSE);
        if ($parent_not_null) {
            $this->db->where('menu.parent_id IS NOT NULL', NULL, FALSE);
        }
        if ($page_is_null) {
    ...
    public function getHeadAll($is_cache = false)
    {
        if ($is_cache) {
            $this->db->cache_on();
        }
    ...
    public function getSlideAll($is_cache = false)
    {
        if ($is_cache) {
            $this->db->cache_on();
        }
