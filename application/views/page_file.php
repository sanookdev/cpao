...
<head>
  <meta charset="utf-8">
  <title><?= $file['file_title'] ?> | <?= $web_name ?></title>
  <meta name="keywords" content="<?= $file['file_title'] ?>,<?= $page['menu_title'] ?>,อบจ.ชลบุรี,องค์การบริหารส่วนจังหวัด,องค์การบริหารส่วนจังหวัดชลบุรี,จังหวัดชลบุรี,ชลบุรี,chonburi,chonburi provincial,chon">
  <meta name="description" content="<?= $file['file_title'] ?> <?= $page['menu_title'] ?>,องค์การบริหารส่วนจังหวัดชลบุรี,chonburi provincial administration organization">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <?php include 'templates/user/style.php'; ?>
  <link href="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.css" media="all" rel="stylesheet" type="text/css" />
            ...
            <div class="row">
              <div class="module-breadcrumb col-sm-12">
                <ul class="breadcrumb">
                  ...
                  <li class="active">
                    <span><a class="active" href="<?= base_url('/page/') ?><?= $page['menu_url'] ?>"><?= $page['menu_title'] ?></a></span>
                  </li>
                </ul>
              </div>
              <section class="module-gallery list-content col-sm-12">
                ...
                <div class="clearfix">
                  <div class="col-xs-12 pannel-sm-head">
                    <i class="fa fa-clock-o"></i> เผยแพร่วันที่ : <?= $page['page_mod_date'] ?>
                  </div>
                </div>
                ...
                <div class="item-content gallery-content">
                  ...
                  if ($page['page_type'] == 'custom') {
                    ...
                    <?php
                      $ext = pathinfo($file['url_download'], PATHINFO_EXTENSION);
                      $icon = '';
                      switch ($ext) {
                        case "pdf":
                        ...
                        case "7z":
                          ...
                          break;
                        default:
                          $icon = '<i class="fa fa-2x fa-file" aria-hidden="true"></i> ';
                      }
                      echo "
                    <div class='row p-4'>
                        <div class='col col-lg-12'>
                        $icon<h3 class='ml-1' style='display: inline;'>" . $file['file_title'] . "</h3>
                        </div>
                        <div class='col col-lg-12 mt-4'>
                        ขนาด " . $file['size'] . "
                        </div>
                        <div class='col col-lg-12 mt-2'>
                        เพิ่มวันที่ " . $file['date_added'] . "
                        </div>
                        <div class='col col-lg-12 mt-2'>
                        จำนวน " . $file['downloads'] . " ครั้ง
                        </div>
                        <div class='col col-lg-12 mt-2'>
                        <a target='_blank' href='" . base_url() . "Download?file=jdownloads/" . trim($cat['cat_dir']) . "/" . $file['url_download'] . "&id=" . $file['file_id'] . "' class='btn btn-primary'><i class='fas fa-download'></i> Download</a>
                        </div>
                    </div>";
                      ?>
                    <?php   } ?>
